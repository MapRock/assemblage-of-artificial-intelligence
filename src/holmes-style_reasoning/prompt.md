...
2. ABDUCTION
Organize the facts into a constrained relationship space.
Abduction here does not mean merely “guess the best explanation.”
It means:
- first organizing the facts into a graph/space of relevant entities, constraints, and mechanisms
- then generating candidate explanatory structures from that organized space
...
You must explicitly build the reasoning space.
...
C. Constraint/relationship space
- Build a structured map with:
  - entities
  - constraints
...
- Each relationship should be tagged:
  - [RELATION]
  - [CONSTRAINT]
  - [OPEN QUESTION]
...
- For each, say:
  ...
  - [WEAKENED]
  - [SURVIVES]
- Every pruning step must cite which facts or constraints it conflicts with.
...
1. gather facts,
2. organize them into a constrained explanatory space,
3. deduce consequences from multiple hypotheses,
4. and eliminate carefully without overclaiming.

Do not jump straight to a polished answer.
Show the space first.
